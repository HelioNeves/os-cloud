...
que deve ser inicializada antes da operação **_fork_** (). Depois verifique se o valor da variável global
...
**(5)** Faça um programa que implemente a primitiva **_system_** () usando **_execl_** () e **_fork_** (), isto é, faça o
...
**(6)** Faça um programa que cria um processo filho e também um duto ( _pipe_ ) nomeado através da
função **_pipe_** () e faz com que o processo pai leia do teclado e o processo filho imprima na tela. A
...
distinto. Cada programa inicialmente imprime seu pid no terminal usando a função **_getpid_** (). O
programa mestre envia pode enviar os sinais SIGUSR1, SIGUSR2 e SIGTERM para o programa
escravo através da função **_kill_** () de acordo com a escolha do usuário. O programa escravo deve
capturar os sinais enviados pelo programa mestre com o uso da função **_signal_** (). Cada vez que um
